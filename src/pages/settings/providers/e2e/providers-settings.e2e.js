...
test('claude preset list shows DeepSeek profile and related env rows', async () => {
  ...
  await expect(win.locator('input[placeholder="KEY"][value="ANTHROPIC_AUTH_TOKEN"]')).toHaveCount(
    1,
  );
  await expect(win.locator('input[placeholder="KEY"][value="ANTHROPIC_BASE_URL"]')).toHaveCount(1);
  await expect(
    win.locator('input[placeholder="输入值"][value="https://api.deepseek.com/anthropic"]'),
  ).toHaveCount(1);
...
test('about section shows contact email and version info', async () => {
  ...
  const aboutPanel = win.getByRole('tabpanel', { name: 'About' });
  await expect(win.getByRole('heading', { name: 'About' })).toBeVisible();
  await expect(aboutPanel.getByText('Cli-Switch').last()).toBeVisible();
  await expect(aboutPanel.getByText('Electron + React + TypeScript')).toBeVisible();
  await expect(aboutPanel.getByText(/0\.1\.[0-9]+/)).toBeVisible();
...
test('claude non-deepseek test accepts ANTHROPIC_AUTH_TOKEN', async () => {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (
      req.method === 'GET' &&
      req.url === '/v1/models' &&
      /^Bearer\s+sk-auth-token-only$/i.test(String(req.headers.authorization || ''))
    ) {
  ...
  try {
    ...
    expect(
      requests.some((item) => item.method === 'GET' && item.url === '/v1/models'),
    ).toBeTruthy();
    expect(
      requests.some((item) => /^Bearer\s+sk-auth-token-only$/i.test(item.authorization)),
    ).toBeTruthy();
