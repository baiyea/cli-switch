...
export const providersBridge = {
  settings: {
    ...
    testProvider(payload: { provider: "claude" | "codex" | "gemini"; profileId: string; envVars: StartupEnvPair[] }) {
      return window.electronAPI.settings.testProvider(payload);
    },
    startProviderOAuthLogin(payload: { provider: "claude" | "codex" | "gemini"; profileId: string; projectId?: string; cwd?: string }) {
      return window.electronAPI.settings.startProviderOAuthLogin(payload);
    },
    probeProviderOAuth(payload: { provider: "claude" | "codex" | "gemini"; profileId: string; envVars: StartupEnvPair[] }) {
      return window.electronAPI.settings.probeProviderOAuth(payload);
    },
    getProviderOAuthLinks(payload: { provider: "claude" | "codex" | "gemini"; profileId?: string; sessionId?: string }) {
      return window.electronAPI.settings.getProviderOAuthLinks(payload);
    },
    testProviderProxy(payload: { provider: "claude" | "codex" | "gemini"; profileId: string; envVars: StartupEnvPair[]; proxyUrl: string }) {
