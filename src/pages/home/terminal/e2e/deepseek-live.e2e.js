...
function buildDeepSeekProviderSettings(token) {
  ...
  return {
    providers: {
      claude: { defaultProfileId: "deepseek-api", enabledProfileId: "deepseek-api", profiles: [deepSeekProfile] },
      codex: { defaultProfileId: "openai-api-key", enabledProfileId: "", profiles: providerEnvPresets.codex.profiles.map((p) => ({ id: p.id, name: p.name, envVars: [] })) },
      gemini: { defaultProfileId: "api-key", enabledProfileId: "", profiles: providerEnvPresets.gemini.profiles.map((p) => ({ id: p.id, name: p.name, envVars: [] })) }
    }
  };
}
  ...
  test("DeepSeek preset live reply", async () => {
    ...
    try {
      ...
      await expect.poll(async () => {
        ...
        return /@anthropic-ai.claude-code.*cli\.js|claude-code.*cli/i.test(text)
