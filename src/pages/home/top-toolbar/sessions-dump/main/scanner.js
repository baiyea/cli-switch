'use strict';

const fs = require('node:fs');
const path = require('node:path');
const os = require('node:os');
const crypto = require('node:crypto');

const PROVIDERS = {
  CLAUDE: 'claude',
  CODEX: 'codex',
  GEMINI: 'gemini',
};

function normalizeProviderId(value) {
  const raw = String(value || '').trim().toLowerCase();
  if (raw === PROVIDERS.CODEX) return PROVIDERS.CODEX;
  if (raw === PROVIDERS.GEMINI) return PROVIDERS.GEMINI;
  return PROVIDERS.CLAUDE;
}

function readHeadLines(filePath, maxBytes = 128 * 1024, maxLines = 300) {
  const stat = fs.statSync(filePath);
  const readSize = Math.min(maxBytes, stat.size);
  const fd = fs.openSync(filePath, 'r');
  try {
    const buffer = Buffer.alloc(readSize);
    fs.readSync(fd, buffer, 0, readSize, 0);
    const text = buffer.toString('utf8');
    return text.split(/\r?\n/).filter(Boolean).slice(0, maxLines);
  } finally {
    fs.closeSync(fd);
  }
}

function readJsonlObjects(filePath, maxBytes = 1024 * 1024) {
  const stat = fs.statSync(filePath);
  const readSize = Math.min(maxBytes, stat.size);
  const fd = fs.openSync(filePath, 'r');
  try {
    const buffer = Buffer.alloc(readSize);
    fs.readSync(fd, buffer, 0, readSize, stat.size - readSize);
    const text = buffer.toString('utf8');
    const lines = text.split(/\r?\n/).filter(Boolean);
    const output = [];
    for (const line of lines) {
      try {
        const parsed = JSON.parse(line);
        if (parsed && typeof parsed === 'object') output.push(parsed);
      } catch {}
    }
    return output;
  } finally {
    fs.closeSync(fd);
  }
}

function normalizeTitle(text, maxLen = 48) {
  if (!text) return '';
  const compact = String(text).replace(/\s+/g, ' ').trim();
  if (!compact) return '';
  if (compact.length <= maxLen) return compact;
  return `${compact.slice(0, maxLen - 1)}…`;
}

function createFallbackTitle(sessionId) {
  return `session-${String(sessionId || '').slice(0, 13)}`;
}

function extractRenameTitle(content) {
  if (typeof content !== 'string') return '';
  if (!content.includes('<command-name>/rename</command-name>')) return '';
  const match = content.match(/<command-args>([\s\S]*?)<\/command-args>/);
  if (!match) return '';
  return normalizeTitle(match[1], 48);
}

function extractPromptTitle(content) {
  if (typeof content !== 'string') return '';
  const trimmed = content.trim();
  if (!trimmed) return '';
  if (trimmed.startsWith('<')) return '';
  if (trimmed.includes('<local-command-caveat>')) return '';
  return normalizeTitle(trimmed, 40);
}

function extractUserPromptFromEvent(parsed) {
  if (!parsed || typeof parsed !== 'object') return '';
  if (parsed?.message?.role === 'user') {
    return extractPromptTitle(parsed?.message?.content);
  }
  if (parsed?.role === 'user') {
    return extractPromptTitle(parsed?.content);
  }
  if (parsed?.type === 'event_msg' && parsed?.payload?.type === 'user_message') {
    return extractPromptTitle(parsed?.payload?.message);
  }
  return '';
}

function findFiles(root, matcher, acc = []) {
  if (!fs.existsSync(root)) return acc;
  const queue = [root];
  while (queue.length > 0) {
    const current = queue.pop();
    let entries = [];
    try {
      entries = fs.readdirSync(current, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const entry of entries) {
      const full = path.join(current, entry.name);
      if (entry.isDirectory()) {
        queue.push(full);
        continue;
      }
      if (entry.isFile() && matcher(full, entry.name)) {
        acc.push(full);
      }
    }
  }
  return acc;
}

function findJsonlLinesFirstCwd(filePath) {
  const head = readHeadLines(filePath);
  for (const line of head) {
    let obj;
    try {
      obj = JSON.parse(line);
    } catch {
      continue;
    }
    const candidates = [
      obj?.cwd,
      obj?.entrypoint?.cwd,
      obj?.payload?.cwd,
      obj?.payload?.entrypoint?.cwd,
      obj?.workdir,
      obj?.workingDirectory,
      obj?.projectPath,
      obj?.project_path,
    ];
    for (const candidate of candidates) {
      if (typeof candidate === 'string' && candidate.trim()) {
        return candidate.trim();
      }
    }
  }
  return '';
}

function findJsonlSessionId(filePath) {
  const head = readHeadLines(filePath);
  for (const line of head) {
    let obj;
    try {
      obj = JSON.parse(line);
    } catch {
      continue;
    }
    if (
      obj?.type === 'session_meta' &&
      typeof obj?.payload?.id === 'string' &&
      obj.payload.id.trim()
    ) {
      return obj.payload.id.trim();
    }
    const candidates = [
      obj?.sessionId,
      obj?.session_id,
      obj?.payload?.sessionId,
      obj?.payload?.session_id,
    ];
    for (const candidate of candidates) {
      if (typeof candidate === 'string' && candidate.trim()) {
        return candidate.trim();
      }
    }
  }
  return '';
}

function deriveJsonlTitle(filePath, fallbackTitle) {
  const lines = readJsonlObjects(filePath, 256 * 1024);
  let promptTitle = '';
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    const parsed = lines[i];
    const content = parsed?.message?.content;
    const rename = extractRenameTitle(content);
    if (rename) return rename;
    if (!promptTitle) {
      const prompt = extractUserPromptFromEvent(parsed);
      if (prompt) promptTitle = prompt;
    }
  }
  return promptTitle || fallbackTitle;
}

function tryReadJson(filePath) {
  try {
    return JSON.parse(fs.readFileSync(filePath, 'utf8'));
  } catch {
    return null;
  }
}

function deepFindStringByKeys(root, keysLower, maxDepth = 8, level = 0) {
  if (level > maxDepth || root == null) return '';
  if (typeof root === 'string') return '';
  if (Array.isArray(root)) {
    for (const item of root) {
      const value = deepFindStringByKeys(item, keysLower, maxDepth, level + 1);
      if (value) return value;
    }
    return '';
  }
  if (typeof root === 'object') {
    for (const [key, value] of Object.entries(root)) {
      if (keysLower.has(String(key).toLowerCase()) && typeof value === 'string' && value.trim()) {
        return value.trim();
      }
    }
    for (const value of Object.values(root)) {
      const nested = deepFindStringByKeys(value, keysLower, maxDepth, level + 1);
      if (nested) return nested;
    }
  }
  return '';
}

function deepFindFirstUserText(root, maxDepth = 10, level = 0) {
  if (level > maxDepth || root == null) return '';
  if (Array.isArray(root)) {
    for (const item of root) {
      const value = deepFindFirstUserText(item, maxDepth, level + 1);
      if (value) return value;
    }
    return '';
  }
  if (typeof root === 'object') {
    const role = root.role || root.author || root.sender;
    if (String(role || '').toLowerCase() === 'user') {
      const text = root.text || root.content || root.prompt || root.message;
      if (typeof text === 'string' && text.trim()) return normalizeTitle(text, 40);
    }
    for (const value of Object.values(root)) {
      const nested = deepFindFirstUserText(value, maxDepth, level + 1);
      if (nested) return nested;
    }
  }
  return '';
}

function readNearestProjectRoot(filePath, stopDir) {
  const stop = path.resolve(stopDir);
  let current = path.resolve(path.dirname(filePath));
  while (current.startsWith(stop)) {
    const marker = path.join(current, '.project_root');
    if (fs.existsSync(marker)) {
      try {
        const value = fs.readFileSync(marker, 'utf8').trim();
        if (value) return value;
      } catch {}
    }
    const parent = path.dirname(current);
    if (parent === current) break;
    current = parent;
  }
  return '';
}

function parseClaudeSession(filePath) {
  const sessionId = path.basename(filePath, '.jsonl');
  const cwd = findJsonlLinesFirstCwd(filePath);
  if (!cwd) return null;
  const stat = fs.statSync(filePath);
  const fallbackTitle = createFallbackTitle(sessionId);
  return {
    provider: PROVIDERS.CLAUDE,
    providerSessionId: sessionId,
    sessionFilePath: filePath,
    cwd: path.resolve(cwd),
    name: deriveJsonlTitle(filePath, fallbackTitle),
    createdAt: stat.mtimeMs,
    sourceMtimeMs: stat.mtimeMs,
    sourceSize: stat.size,
  };
}

function parseCodexSession(filePath) {
  const sessionId = findJsonlSessionId(filePath) || path.basename(filePath, '.jsonl');
  const cwd = findJsonlLinesFirstCwd(filePath);
  if (!cwd) return null;
  const stat = fs.statSync(filePath);
  const fallbackTitle = createFallbackTitle(sessionId);
  return {
    provider: PROVIDERS.CODEX,
    providerSessionId: sessionId,
    sessionFilePath: filePath,
    cwd: path.resolve(cwd),
    name: deriveJsonlTitle(filePath, fallbackTitle),
    createdAt: stat.mtimeMs,
    sourceMtimeMs: stat.mtimeMs,
    sourceSize: stat.size,
  };
}

function parseGeminiSession(filePath) {
  const payload = tryReadJson(filePath);
  if (!payload) return null;
  const sessionId = String(
    payload.sessionId || payload.session_id || path.basename(filePath, '.json'),
  ).trim();
  const cwd =
    deepFindStringByKeys(
      payload,
      new Set(['cwd', 'workdir', 'workingdirectory', 'projectpath', 'project_path', 'rootdir']),
    ) || readNearestProjectRoot(filePath, path.join(os.homedir(), '.gemini', 'tmp'));
  if (!cwd) return null;

  const stat = fs.statSync(filePath);
  const title = deepFindFirstUserText(payload) || createFallbackTitle(sessionId);
  return {
    provider: PROVIDERS.GEMINI,
    providerSessionId: sessionId,
    sessionFilePath: filePath,
    cwd: path.resolve(cwd),
    name: title,
    createdAt: stat.mtimeMs,
    sourceMtimeMs: stat.mtimeMs,
    sourceSize: stat.size,
  };
}

function listProviderSessions(homeDir = os.homedir()) {
  const roots = {
    [PROVIDERS.CLAUDE]: path.join(homeDir, '.claude', 'projects'),
    [PROVIDERS.CODEX]: path.join(homeDir, '.codex', 'sessions'),
    [PROVIDERS.GEMINI]: path.join(homeDir, '.gemini', 'tmp'),
  };
  const all = [];

  const claudeFiles = findFiles(roots[PROVIDERS.CLAUDE], (full, name) => {
    if (!name.endsWith('.jsonl')) return false;
    const normalized = String(full).replace(/\\/g, '/');
    if (normalized.includes('/subagents/')) return false;
    if (name.startsWith('agent-')) return false;
    return true;
  });
  for (const filePath of claudeFiles) {
    const item = parseClaudeSession(filePath);
    if (item) all.push(item);
  }

  const codexFiles = findFiles(roots[PROVIDERS.CODEX], (_full, name) => name.endsWith('.jsonl'));
  for (const filePath of codexFiles) {
    const item = parseCodexSession(filePath);
    if (item) all.push(item);
  }

  const geminiFiles = findFiles(
    roots[PROVIDERS.GEMINI],
    (full, name) => name.endsWith('.json') && full.includes(`${path.sep}chats${path.sep}`),
  );
  for (const filePath of geminiFiles) {
    const item = parseGeminiSession(filePath);
    if (item) all.push(item);
  }

  const normalized = all
    .filter((item) => item && item.provider && item.providerSessionId && item.cwd)
    .map((item) => ({ ...item, provider: normalizeProviderId(item.provider) }));

  const dedupedMap = new Map();
  for (const item of normalized) {
    const key = `${item.provider}:${item.providerSessionId}`;
    const prev = dedupedMap.get(key);
    if (!prev || (item.createdAt || 0) >= (prev.createdAt || 0)) {
      dedupedMap.set(key, item);
    }
  }

  return Array.from(dedupedMap.values()).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
}

function isPathSameOrInside(candidate, rootPath) {
  const candidateAbs = path.resolve(candidate);
  const rootAbs = path.resolve(rootPath);
  if (candidateAbs === rootAbs) return true;
  const rel = path.relative(rootAbs, candidateAbs);
  return !!rel && !rel.startsWith('..') && !path.isAbsolute(rel);
}

function extractTextFromContentValue(content) {
  if (typeof content === 'string') return content.trim();
  if (!content) return '';

  if (Array.isArray(content)) {
    return content
      .map((item) => {
        if (typeof item === 'string') return item;
        if (!item || typeof item !== 'object') return '';
        if (typeof item.text === 'string') return item.text;
        if (typeof item.output_text === 'string') return item.output_text;
        if (typeof item.input_text === 'string') return item.input_text;
        if (typeof item.content === 'string') return item.content;
        if (item.type === 'text' && typeof item.value === 'string') return item.value;
        return '';
      })
      .filter(Boolean)
      .join('\n')
      .trim();
  }

  if (typeof content === 'object') {
    if (typeof content.text === 'string') return content.text.trim();
    if (typeof content.output_text === 'string') return content.output_text.trim();
    if (typeof content.input_text === 'string') return content.input_text.trim();
    if (typeof content.content === 'string') return content.content.trim();
    if (typeof content.value === 'string') return content.value.trim();
    if (Array.isArray(content.parts)) {
      const joined = content.parts
        .map((part) => (typeof part?.text === 'string' ? part.text : ''))
        .filter(Boolean)
        .join('\n')
        .trim();
      if (joined) return joined;
    }
  }

  return '';
}

function extractMessageTextBlocks(content = []) {
  if (!Array.isArray(content)) return '';
  return content
    .map((item) => {
      if (!item || typeof item !== 'object') return '';
      if (typeof item.text === 'string') return item.text;
      if (typeof item.input_text === 'string') return item.input_text;
      if (typeof item.output_text === 'string') return item.output_text;
      if (typeof item.content === 'string') return item.content;
      return '';
    })
    .filter(Boolean)
    .join('\n')
    .trim();
}

function extractConversationText(content) {
  if (typeof content === 'string') return String(content).trim();
  if (Array.isArray(content)) {
    return content
      .map((item) => {
        if (!item || typeof item !== 'object') return '';
        const itemType = String(item.type || '').toLowerCase();
        if (itemType === 'tool_result' || itemType === 'tool_use') return '';
        if (typeof item.text === 'string') return item.text;
        if (typeof item.content === 'string') return item.content;
        if (typeof item.thinking === 'string') return item.thinking;
        return extractTextFromContentValue(item);
      })
      .filter(Boolean)
      .join('\n')
      .trim();
  }
  return extractTextFromContentValue(content);
}

function isSkippableConversationText(text = '') {
  const value = String(text || '').trim();
  if (!value) return true;
  if (/^\[Request interrupted by user\]$/i.test(value)) return true;
  if (/^No files found$/i.test(value)) return true;
  if (/^Found \d+ file/i.test(value)) return true;
  return false;
}

function toRoundSignature(provider, providerSessionId, roundIndex, userText, assistantText) {
  const hash = crypto
    .createHash('sha1')
    .update(`${provider}\n${providerSessionId}\n${roundIndex}\n${userText}\n${assistantText}`)
    .digest('hex')
    .slice(0, 16);
  return `${roundIndex}:${hash}`;
}

function buildRoundsFromTurns(provider, providerSessionId, turns = []) {
  const rounds = [];
  let pendingUser = '';
  for (const turn of turns) {
    if (!turn || typeof turn !== 'object') continue;
    const role = String(turn.role || '').toLowerCase();
    const text = String(turn.text || '').trim();
    if (!text) continue;

    if (role === 'user') {
      pendingUser = text;
      continue;
    }

    if (role !== 'assistant') continue;
    if (!pendingUser) continue;

    const roundIndex = rounds.length + 1;
    const userText = pendingUser;
    const assistantText = text;
    rounds.push({
      roundIndex,
      userText,
      assistantText,
      signature: toRoundSignature(provider, providerSessionId, roundIndex, userText, assistantText),
    });
    pendingUser = '';
  }
  return rounds;
}

function parseClaudeRounds(sessionFilePath, providerSessionId) {
  const events = readJsonlObjects(sessionFilePath, 2 * 1024 * 1024);
  const turns = [];
  for (const parsed of events) {
    const role = String(parsed?.message?.role || parsed?.role || parsed?.type || '').toLowerCase();
    if (role !== 'user' && role !== 'assistant') continue;
    if (parsed?.isMeta) continue;
    const text = extractConversationText(parsed?.message?.content ?? parsed?.content);
    if (!isSkippableConversationText(text)) turns.push({ role, text });
  }
  return buildRoundsFromTurns(PROVIDERS.CLAUDE, providerSessionId, turns);
}

function parseCodexRounds(sessionFilePath, providerSessionId) {
  const events = readJsonlObjects(sessionFilePath, 2 * 1024 * 1024);
  const turns = [];
  for (const parsed of events) {
    if (parsed?.type === 'event_msg' && parsed?.payload?.type === 'user_message') {
      const text = String(parsed?.payload?.message || '').trim();
      if (text) turns.push({ role: 'user', text });
      continue;
    }
    if (parsed?.type === 'response_item' && parsed?.payload?.type === 'message') {
      const role = String(parsed?.payload?.role || '').toLowerCase();
      if (role !== 'user' && role !== 'assistant') continue;
      const text = extractMessageTextBlocks(parsed?.payload?.content || []);
      if (text) turns.push({ role, text });
      continue;
    }
    const directRole = String(parsed?.message?.role || parsed?.role || '').toLowerCase();
    if (directRole !== 'user' && directRole !== 'assistant') continue;
    const text = extractConversationText(parsed?.message?.content ?? parsed?.content);
    if (!isSkippableConversationText(text)) turns.push({ role: directRole, text });
  }
  return buildRoundsFromTurns(PROVIDERS.CODEX, providerSessionId, turns);
}

function parseGeminiRounds(sessionFilePath, providerSessionId) {
  const payload = tryReadJson(sessionFilePath);
  if (!payload) return [];
  const turns = [];
  const messages = Array.isArray(payload?.messages) ? payload.messages : [];
  for (const message of messages) {
    const type = String(message?.type || '').toLowerCase();
    if (type !== 'user' && type !== 'gemini') continue;
    const text = extractTextFromContentValue(message?.content);
    if (!text) continue;
    turns.push({ role: type === 'gemini' ? 'assistant' : 'user', text });
  }
  return buildRoundsFromTurns(PROVIDERS.GEMINI, providerSessionId, turns);
}

function readSessionRounds(session) {
  const provider = normalizeProviderId(session?.provider);
  const providerSessionId = String(session?.providerSessionId || '').trim();
  const sessionFilePath = String(session?.sessionFilePath || '').trim();
  if (!providerSessionId || !sessionFilePath || !fs.existsSync(sessionFilePath)) {
    return [];
  }
  if (provider === PROVIDERS.CLAUDE) return parseClaudeRounds(sessionFilePath, providerSessionId);
  if (provider === PROVIDERS.CODEX) return parseCodexRounds(sessionFilePath, providerSessionId);
  if (provider === PROVIDERS.GEMINI) return parseGeminiRounds(sessionFilePath, providerSessionId);
  return [];
}

function listProjectProviderSessions({ projectPath, homeDir = os.homedir() }) {
  const targetProjectPath = path.resolve(projectPath);
  return listProviderSessions(homeDir).filter((session) => {
    if (!session?.cwd) return false;
    return isPathSameOrInside(session.cwd, targetProjectPath);
  });
}

module.exports = {
  PROVIDERS,
  normalizeProviderId,
  listProjectProviderSessions,
  readSessionRounds,
};
