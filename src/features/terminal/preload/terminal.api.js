...
function createTerminalApi() {
  return {
    terminal: {
      start: (payload) => ipcRenderer.invoke(TERMINAL_CHANNELS.START, payload),
      snapshot: (payload) => ipcRenderer.invoke(TERMINAL_CHANNELS.SNAPSHOT, payload),
      write: (payload) => ipcRenderer.send(TERMINAL_CHANNELS.WRITE, payload),
      resize: (payload) => ipcRenderer.send(TERMINAL_CHANNELS.RESIZE, payload),
      kill: (payload) => ipcRenderer.send(TERMINAL_CHANNELS.KILL, payload),
      onData: (listener) => {
