...
const { buildSchemaSql } = require("../../src/kernel/db/models");
...
function loadDeepSeekToken() {
  ...
  if (!token) {
    throw new Error(".env 缺少 TEST_DEEPSEEK，无法运行 DeepSeek Claude Code live e2e");
  }
  return token;
}
...
async function launchApp() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "cliswitch-claude-settings-live-"));
...
async function waitForClaudeCodeReady(win, sessionId) {
  await expect.poll(async () => {
    ...
    if (/No startup command available|stdin is not a terminal|command not found/i.test(text)) {
      throw new Error(`Claude Code did not start correctly: ${text.slice(-2000)}`);
    }
    return /@anthropic-ai\\claude-code\\cli\.js|@anthropic-ai\/claude-code\/cli\.js|claude/i.test(text);
...
async function sendPromptAndWaitForReply(win, sessionId, prompt, marker) {
  ...
  await expect.poll(async () => {
    ...
    if (/invalid api key|401|Unauthorized|No startup command available|stdin is not a terminal/i.test(text)) {
      throw new Error(`Claude DeepSeek terminal failed: ${text.slice(-2500)}`);
    }
...
test("Settings 配置 DeepSeek key 后 Claude Code 终端能收到完整回复", async () => {
  test.setTimeout(9 * 60 * 1000);
  const token = loadDeepSeekToken();
  ...
  try {
    ...
    await configureDeepSeekInSettings(win, token);
