...
test("Ctrl+Shift+V or Cmd+Shift+V should not trigger copy/paste handler", async () => {
  ...
  if (comboCheck.shiftCPrevented || comboCheck.shiftVPrevented) {
    console.log("[e2e] WARNING: Shift+Ctrl/Cmd combinations are being intercepted by attachCustomKeyEventHandler");
    console.log("[e2e] This may cause issues with custom terminal shortcuts on", comboCheck.platform);
  }
