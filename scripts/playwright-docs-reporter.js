...
class DocsReporter {
  ...
  onEnd(result) {
    ...
    if (this.failures.length === 0) {
      summaryLines.push("", "No individual test failures were captured, but the run did not complete cleanly.");
    }
...
function writeCaseDetail(failure) {
  ...
  if (failure.errors.length > 0) {
    ...
    failure.errors.forEach((error, index) => {
      lines.push(`### Error ${index + 1}`, "", "```text", error.message || "Unknown error", "```", "");
    });
  }
  ...
  if (failure.attachments.length > 0) {
    ...
    failure.attachments.forEach((attachment) => {
      const relativePath = path.relative(e2eArtifactsRoot, attachment.path).split(path.sep).join("/");
