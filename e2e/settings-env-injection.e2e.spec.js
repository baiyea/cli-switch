...
test("save settings then create session should inject env vars into claude process", async () => {
  ...
  await win.getByPlaceholder("https://api.anthropic.com").fill(apiUrl);
  await win.getByPlaceholder("sk-...").fill(apiKey);
  await win.locator("select").selectOption("ANTHROPIC_AUTH_TOKEN");
  await win.getByPlaceholder("claude-3-5-sonnet-20241022").fill(model);

  await win.getByRole("button", { name: /新增变量/ }).click();
  const lastEnvRow = win.locator(".env-row").last();
  await lastEnvRow.locator("input").nth(0).fill(extraKey);
  await lastEnvRow.locator("input").nth(1).fill(extraValue);
  ...
  await win.getByRole("button", { name: "+ 新建会话" }).click();
